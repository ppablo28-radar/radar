...
// ─── Escala visual: índice 0-11 ─────────────────────────────
// [VT][V2][V1] | [E] | [N1][N2][N3][N4][N5][N6][N7][N8]
const SCALE = [
...
const parseActiveIndex = (nivelCompra, nivelVenta) => {
  ...
  const match = c.match(/NIVEL\s*(\d+)/);
  if (match) {
    const n = parseInt(match[1]);
    if (n >= 1 && n <= 8) return 3 + n; // Nivel 1 → idx 4 … Nivel 8 → idx 11
  }
  return 3; // Esperar
};

// ─── Componente LevelBar ─────────────────────────────────────
function LevelBar({ nivelCompra, nivelVenta }) {
          ...
          return (
            <div
              ...
              style={{
                ...
                boxShadow: isActive
                  ? `0 0 10px ${step.color}cc, 0 0 20px ${step.color}55`
                  : "none",
                flexShrink: 0,
                transition: "all 0.2s",
              }}
...
const ACTION_CONFIG = {
  ...
  NEUTRAL:     { bg: "linear-gradient(135deg,#0f172a,#1e293b)", border: "#334155", label: "ESPERAR",     textColor: "#94a3b8" },
};
    ...
    <div style={{ minHeight: "100vh", background: "#030712", color: "#e2e8f0", fontFamily: "'DM Mono','Courier New',monospace" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .stat-card{background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:14px 20px}
      ...
      <div style={{ borderBottom: "1px solid #1e293b", padding: "24px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        <div>
          <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 28, fontWeight: 800, letterSpacing: "-0.04em", display: "flex", alignItems: "baseline", gap: 4 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 400, color: "#475569", marginLeft: 12, letterSpacing: "0.1em" }}>v2.2</span>
          </div>
          <div style={{ color: "#475569", fontSize: 11, letterSpacing: "0.15em", marginTop: 4, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {error
              ...
              : lastUpdate && <span style={{ display: "flex", alignItems: "center", gap: 4 }}><Wifi size={12} style={{ color: "#10b981" }}/>{lastUpdate.toLocaleTimeString("es-AR")}</span>
            }
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <button className="btn" onClick={() => fetchData(true)} style={{ background: "#0f172a", border: "1px solid #1e293b", color: "#64748b", padding: "10px 14px" }}>
          ...
          {[
            ...
            <div key={s.l} className="stat-card" style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "#475569", fontSize: 10, letterSpacing: "0.12em" }}>{s.l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* TOOLBAR */}
      <div style={{ padding: "16px 32px", borderBottom: "1px solid #0f172a", display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }} className="top-bar">
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value)}
            style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 6, color: "#94a3b8", padding: "6px 10px", fontSize: 12, outline: "none" }}>
      ...
      <div style={{ padding: "24px 32px" }}>
        {loading && (
          <div style={{ textAlign: "center", padding: "80px 0", color: "#334155", fontSize: 13, letterSpacing: "0.15em" }}>
              ...
              {filtered.map(item => {
                ...
                return (
                  <div key={item.ticker} className="card"
                    ...
                    {/* ── BARRA DE NIVELES ── */}
                    <div style={{ background: "#00000033", borderRadius: 8, padding: "10px 12px", marginBottom: 12 }}>
                      <LevelBar nivelCompra={item.nivelCompra} nivelVenta={item.nivelVenta} />
                    </div>
